 ...
 **/
 ...
 cmd({
	...
	const Avatar = "Avatar"
	const daily  = await eco.daily(citel.sender, Avatar, 2000); //give 500 for daily, can be changed
...
Version: 0.0.6`, citel);
 ...
 cmd({
	...
	if(!users) return citel.reply('Please give me user.')
        const balance  = await eco.balance(users, "Avatar")
        await eco.deduct(users, "Avatar", balance.wallet);
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const balance  = await eco.balance(user, Avatar)
    switch (value) {
        case '1000':
        case '1':
        if (k > balance.wallet ) return citel.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
          const deduct1 = await eco.deduct(user, Avatar, 100);
          const add1 = eco.giveCapacity(user, Avatar, 1000);
              let buttons = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
          const deduct2 = await eco.deduct(user, Avatar, 1000);
          const add2 = eco.giveCapacity(user, Avatar, 100000);
              let buttonss = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
           const deduct3 = await eco.deduct(user, Avatar, 10000);
           const add3 = eco.giveCapacity(user, Avatar, 10000000);
               let buttonst = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
 ...
 await citel.reply('*What are you trying to do📉*.')
    ...
    async(Void, citel, text,{ isCreator }) => {
      ...
      //  let users = citel.mentionedJid ? citel.mentionedJid[0] : citel.msg.contextInfo.participant || false;
        ...
        let d = parseInt(text)
        const deposit = await eco.deposit(citel.sender, "Avatar", d);
        const balance = await eco.balance(citel.sender, "Avatar")
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const Avatar = "Avatar"
        ...
        const balance = await eco.balance(user1, Avatar);
        let a = (balance.wallet) < parseInt(word)
        //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        if(a == true) return citel.reply("you dont have sufficient money to transfer👎");

        const deduct = await eco.deduct(user1, Avatar, value[0]);
        const give = await eco.give(user2, Avatar, value[0]);
        let buttons = [{
            buttonId: `${prefix}wallet`,
            buttonText: {
                displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
         const Avatar = "Avatar"
         const balance = await eco.balance(citel.sender, Avatar); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
         let buttons = [{
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        if(!isCreator) return
         ...
         await eco.give(users, Avatar, parseInt(text.split(' ')[0]));
...
Version: 0.0.6`, citel);
	...
	const Avatar = "Avatar"
	    const k = 1000
        const balance1  = await eco.balance(user1, Avatar)
	const balance2  = await eco.balance(user2, Avatar)
    ...
    switch (random) {
	  ...
	  const deduff = Math.floor(Math.random() * 1000)	    
          await eco.deduct(user2, Avatar, deduff);
          await eco.give(citel.sender, Avatar, deduff);
        ...
        case 'caught':
           ...
           await eco.deduct(user1, Avatar, rmoney);
...
default:
 ...
 ///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
     ...
     if(opp==='left')
     {
         hjkl = 'https://github.com/AvatarBot/Brandimages/blob/main/Nezuko/leftr.webp?raw=true'
     } 
    else if(opp==='right') 
    {
        hjkl = 'https://github.com/AvatarBot/Brandimages/blob/main/Nezuko/rightr.webp?raw=true'
    } else if(opp==='up') 
    {
        hjkl = 'https://github.com/AvatarBot/Brandimages/blob/main/Nezuko/upr.webp?raw=true'
    } else if (opp==='down'){
        hjkl = 'https://github.com/AvatarBot/Brandimages/blob/main/Nezuko/downr.webp?raw=true'
    } else{
        citel.reply(`Please provide direction(left,right,up,down).\nEg:- ${prefix}gamble 200 left`)
    }
   let media = await getBuffer(hjkl)
   citel.reply(media,{packname:'Avatar',author:'Economy'},"sticker")
     ...
     if (!text) return citel.reply(
        ...
        if ( r == opp){
           let give = await eco.give(user , Avatar, twice);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
...
Version: 0.0.6`, citel);
           ...
           let deduct = await eco.deduct(user, Avatar, texts[0]);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
            ...
            const Avatar = "Avatar"
            const k = 100
            const balance1  = await eco.balance(user, Avatar)
...
if(value<=balance.wallet){
    ...
    if ((f1 !== f2) && f2 !== f3){
        const deduct1 = await eco.deduct(user, Avatar, deduff);
        return citel.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && f2 == f3){
        const give1 = await eco.give(user, Avatar, deduff/2);
        return citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
     }
     else if ((f1 == f2) && f2 !== f3){
        const give2 = await eco.give(user, Avatar, deduff);
        return citel.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f1 == f3){
        const deduct2 = await eco.deduct(user, Avatar, deduff);
        return citel.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f2 == f3){
        const give4 = eco.give(user, Avatar, deduff);
        return citel.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
        const give5 = eco.give(user, Avatar, deduff*20);
    ...
    return citel.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
            ...
            if ((f1 !== f2) && f2 !== f3){
               const deduct1 = await eco.deduct(user, Avatar, 50);
                      citel.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
            }
            else if ((f1 == f2) && f2 == f3){
               const give1 = await eco.give(user, Avatar, 100);
                     citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
            }
            else if ((f1 == f2) && f2 !== f3){
               const give2 = await eco.give(user, Avatar, 20);
                     citel.reply(`${mess3}\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f1 == f3){
               const deduct2 = await eco.deduct(user, Avatar, 20);
                     citel.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f2 == f3){
               const give4 = eco.give(user, Avatar, 20);
                     citel.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
               const give5 = eco.give(user, Avatar, 1000);
                    citel.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
            }
            else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "Avatar")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "Avatar", deduff*2);
...
Version: 0.0.6`, citel);
    ...
    const deduct1 = await eco.deduct(citel.sender, "Avatar", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
    let buttons = [{
        buttonId: `${prefix}slot`,
