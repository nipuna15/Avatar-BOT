	...
	return new Promise((resolve, reject) => {	
		...
		axios({	
			...
			axios({	
				...
				resolve({	
					status: true,	
					message: "Created By Avatar Botto",	
					result: result	
				})	
			}).catch(reject)	
		}).catch(reject)	
...
if (fs.existsSync('./Themes/' + Config.LANG + '.json')) {	
	...
	var json = JSON.parse(fs.readFileSync('./Themes/Avatar.json'));	
}	
...
const picAvatarh = todlink[Math.floor(Math.random() * todlink.length)];	
resolve(picAvatarh)	
})	
 }	
